...
export default async function setupWebRTC(state) {
  if (!WebRTC.isSupported()) return null;

  const config = PEER_CONFIG;
  ...
  const webrtc = new WebRTC({
    ...
    peerSettings: {
      trickle: true,
      sdpTransform: (sdp) => {
        console.log('sdpTransform', state.bandwidth); // , sdp)
        let newSDP = sdp;
        if (state.bandwidth) {
  ...
  const onNegotiateBandwidth = messages.on('negotiateBandwidth', () => {
    webrtc.forEachPeer((peer) => {
      peer.peer.negotiate();
    });
  });
